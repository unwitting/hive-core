...
class Board {
  ...
  get clone() {
    const newBoard = new Board(this._opts)
  ...
  applyMove(move, turn) {
    ...
    if (move.isMovement) {
      ...
      let isPureSlide = false
      if ((_.includes(['Q', 'A', 'S'], move.piece)) ||
          (move.piece === 'B' && (origin[2] === move.movementTargetCoords[2]))) {
        isPureSlide = true
      }
      // Compare intended around-hive move against valid paths for sliders
      if (isPureSlide) {
        let paths = this.getPathsAroundHive(origin, move.movementTargetCoords)
        if (move.piece === 'Q') {
        ...
        if (move.piece === 'B') {
          onHive = origin[2] > 0
          // TODO beetle movement here should be done with a 'getPathsAroundAndOverHive' function in the same
          // way as slider movement is done - this is a bit of a hack
          if (!onHive) { paths = _.filter(paths, p => p.length === 2) }
          else {
            if (!this._cellsAreAdjacent(origin, move.movementTargetCoords)) {
              throw new MoveValidationError('NO_VALID_PATH')
            }
            const occupiedSharedNeighbours = _(this._getSharedNeighbours(origin, move.movementTargetCoords))
              .filter(n => this._getCell(n).length > 0).value()
            if (occupiedSharedNeighbours.length === 2) {
              throw new MoveValidationError('NO_VALID_PATH')
            }
          }
        }
        if (paths.length === 0 && !onHive) {
          throw new MoveValidationError('NO_VALID_PATH')
        }
      }
  ...
  getPathsAroundHive(origin, dest, _currentPath=null) {
    ...
    const validNextSteps =
      ...
      // Filter out cells without exactly one occupied shared neighbour (two means no freedom to move through,
      // none means you're jumping a gap)
      .reject(({coords2}) => {
        const sharedNeighbours = this._getSharedNeighbours(_currentPath[_currentPath.length - 1], coords2)
        return _(sharedNeighbours).filter(n => this._getCell(n).length > 0).value().length !== 1
      })
      .value()
    ...
    for (const {coords2} of validNextSteps) {
      ...
      const pathFinishers = nextStepBoard.getPathsAroundHive(coords2, dest, newCurrentPath)
