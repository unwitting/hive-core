...
class Board {
  ...
  applyMove(move, turn) {
    ...
    if (_.isString(move)) { move = new Move(move) }
    const { origin } = this._validateMove(move, turn)
    // Apply move
  ...
  getPathsAroundHive(origin, dest, stopTryingCb=(() => false) , allowClimbing=false, _currentPath=null) {
    if (_currentPath === null) { _currentPath = [origin] }
    if (stopTryingCb(_currentPath)) { return null }
  ...
  getValidMovementTargets(originCoords3, turn) {
    ...
    for (const occupiedOrAdjacentCoord2 of this._occupiedAndAdjacentCoord2s) {
      ...
      try {
        this._validateMove(moveString, turn)
