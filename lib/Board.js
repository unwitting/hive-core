...
class Board {
  ...
  getPathsAroundHive(origin, dest, currentPath=null) {
    if (currentPath === null) { currentPath = [origin] }
    const adjacentCells = this.getAdjacentCells(origin, true)
    // Else, trawl through the adjacents for something valid
    const validNextSteps = _
      .chain(adjacentCells)
      ...
      .reject(({coords2}) => {
        return _.filter(this.getAdjacentCells(coords2, true), c => {
          ...
          if (adjCoords[0] === currentPath[0][0] && adjCoords[1] === currentPath[0][1]) {
            return true
          }
          return false
        }).length === 6
      ...
      .reject(({coords2}) => !!_.find(currentPath, c => (c[0] === coords2[0] && c[1] === coords2[1])))
      ...
      .reject(({coords2}) => {
        const sharedNeighbours = this._getSharedNeighbours(currentPath[currentPath.length - 1], coords2)
        return _.chain(sharedNeighbours).filter(n => this._getCell(n).length > 0).value().length !== 1
      })
      .value()
    ...
    for (const {coords2} of validNextSteps) {
      const newCurrentPath = JSON.parse(JSON.stringify(currentPath))
      newCurrentPath.push(coords2)
      // Create a board in which the move to the next step has been made
      const nextStepBoard = this.clone
      nextStepBoard._moveTopToTopOfCell(currentPath[currentPath.length - 1], coords2)
  ...
  _getSharedNeighbours(coordsA, coordsB) {
    ...
    return _.chain(adjA).filter(adjCellA => {
