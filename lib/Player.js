...
class RandomBotPlayer extends BotPlayer {
  ...
  async move(game) {
    ...
    if (this.pieces.length > 0 && validPlacementLocations.length > 0) {
      let piece = this.randomPiece
      if ((game.turn === 7 || game.turn === 8) && _.includes(this.pieces, 'Q')) {
        piece = 'Q'
      }
      const placementCoords = validPlacementLocations[_.random(0, validPlacementLocations.length - 1)]
      return `${piece}+${placementCoords.join(',')}`
    }
