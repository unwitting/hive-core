...
describe('applyMove', () => {
  ...
  test('disallows a second move not at 1,0', () => {
    const board = new Board()
    board.applyMove('S+0,0', 1)
    expect(() => board.applyMove('Q+0,1', 2)).toThrowError('SECOND_MOVE_PRIMARY_REQUIRED')
  })
  ...
  test.only('disallows movement of piece trapped under others', () => {
    const board = new Board()
    board.applyMove('A+0,0', 1)
    board.applyMove('A+1,0', 2)
    board.applyMove('Q+-1,1', 3)
    ...
    board.applyMove('B>-1,1', 7)
    board.applyMove('B>1,1', 8)
...
describe('getAdjacentCells', () => {
  ...
  test('successfully gets primary-filled adjacent cells', () => {
    const board = new Board()
    board.applyMove('S+0,0', 1)
    board.applyMove('Q+1,0', 2)
    expect(board.getAdjacentCells([0, 0])).toEqual([ [{ piece: 'Q', white: false }], [], [], [], [], [] ])
  })
})
...
describe('piecesPlaced', () => {
  ...
  test('returns correctly after some valid moves', () => {
    ...
    expect(board.piecesPlaced).toEqual({
      ...
      black: [ { piece: 'B', coords3: [2, 0, 0] } , { piece: 'Q', coords3: [1, 0, 0] } ],
    })
  })
})
